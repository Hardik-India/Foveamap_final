...
export function extractObjects(points:Point[],semantic?:number[],names?:Record<number,string>,cfg:TrackingConfig=TRACKING_DEFAULTS):TrackObject[]{
  ...
  for(let i=0;i<points.length;i++){
    const terrain=semantic&&[0,1,3,10,23,31,33].includes(semantic[i]);
  ...
  while(bins.size&&out.length<80){
    ...
    if(semantic){const counts=new Map<number,number>();for(const i of ids)counts.set(semantic[i],(counts.get(semantic[i])||0)+1);const best=[...counts.entries()].sort((a,b)=>b[1]-a[1])[0]?.[0];if(best!==undefined)obj.className=names?.[best]||`rellis-${best}`;}
    out.push(obj);
  }
